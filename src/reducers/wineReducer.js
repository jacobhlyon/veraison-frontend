function wineReducer(state ={allWines: {}}, action) {
	switch(action.type) {
		...
		case "PERSISTED_WINE":
			localStorage.setItem("currentWineObject", JSON.stringify(action.payload.id))
			return Object.assign({}, state, {currentWine: action.payload})
		case "CREATED_WINE_SCORE":
			localStorage.setItem("currentWineScoreId", JSON.stringify(action.payload.id))
		...
		case "FETCHED_ALL_SCORES_FOR_WINE":
			...
			localStorage.setItem("currentWineScoreId", JSON.stringify(action.payload.allScores.sight_score[0].wine_score_id))
